...
class DGITrainer(Trainer):
    ...
    def predict(self, batch):
        ...
        Feature = torch.cat((User_feature, Item_feature),
                            dim=-1)
        Feature = self.gen.edge_maker.encoder(Feature)
    ...
    def reconstruct(self, UV, VU, UV_rated, VU_rated, relation_UV_adj, relation_VU_adj, adj,CUV, CVU,fake_adj, batch):
        ...
        decoded = self.gen.edge_maker(torch.cat((user_feature_Two, item_feature_Two), dim=1))
        neg_decoded = self.gen.edge_maker(torch.cat((user_feature_Two, neg_item_feature_Two), dim=1))
        Auto_pos_loss = self.autoEncoderLoss(decoded, torch.cat((user_feature_Two, item_feature_Two), dim=1)) 
        Auto_neg_loss = self.autoEncoderLoss(neg_decoded, torch.cat((user_feature_Two, neg_item_feature_Two), dim=1))
        Auto_loss = (Auto_pos_loss + Auto_neg_loss)/2

        temp_pos = self.gen.edge_maker.encoder(torch.cat((user_feature_Two, item_feature_Two), dim=1))
        temp_neg = self.gen.edge_maker.encoder(torch.cat((user_feature_Two, neg_item_feature_Two), dim=1))
        pos_One = self.gen.score(temp_pos)
        neg_One = self.gen.score(temp_neg) 
        ...
        # lossG = lossG_real
        lossG2 = (1 - self.opt["lambda"] - self.opt["beta"]) * reconstruct_loss + self.opt["lambda"] * lossG + self.opt["beta"] * Auto_loss
